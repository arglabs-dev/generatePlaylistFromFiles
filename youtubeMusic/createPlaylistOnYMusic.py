...
# --- CONSTANTS ---
LOGS_DIR = "logs"
LOG_FILENAME = os.path.join(LOGS_DIR, f"migration_log_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
REPORT_FILENAME = os.path.join(LOGS_DIR, f"migration_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.md")
DEFAULT_REQUEST_DELAY = 0.5  # Default delay in seconds between API requests
...
class MusicMigrator:
    def __init__(self, json_path: str, playlist_name: str, music_dir: Optional[str] = None, request_delay: float = DEFAULT_REQUEST_DELAY):
    ...
    def get_or_create_playlist(self) -> str:
        ...
        try:
            ...
            logging.debug(f"Successfully retrieved {len(playlists)} playlists")
        except Exception as e:
            logging.error(f"Failed to read playlist library (API Error: {e}).")
            logging.info("Assuming playlist doesn't exist and creating a new one...")
            playlists = []
        ...
        try:
            playlist_id = self.yt.create_playlist(self.playlist_name, "Auto-generated by MusicMigrator")
            logging.info(f"Playlist created successfully. ID: {playlist_id}")
            return playlist_id
        except Exception as e:
            logging.critical(f"FATAL: Could not create playlist. Verify that your YTM channel exists. Error: {e}")
            raise
    ...
    def search_song(self, artist: str, title: str, album: str) -> Optional[str]:
        try:
            ...
            if results: 
                logging.debug(f"Found song with specific query: {query}")
            ...
            results = self.yt.search(query, filter='songs')
            if results: 
                logging.debug(f"Found song with general query: {query}")
                return results[0]['videoId']
                
            logging.warning(f"Song not found: {artist} - {title}")
            return None
        except Exception as e:
            logging.error(f"Error searching for song '{artist} - {title}': {e}")
            return None
    ...
    def process(self):
        ...
        for song in songs:
            ...
            # Add delay between individual song processing to avoid rate limiting
            ...
            if len(ids_to_add) >= 20:
                try:
                    ...
                    logging.info(f"Successfully added {len(ids_to_add)} songs to playlist")
                    ids_to_add = []
                    # Add delay between batch requests
                    if self.request_delay > 0:
                        time.sleep(self.request_delay)
                except Exception as e:
                    logging.error(f"Failed to add songs to playlist (batch): {e}")
        ...
        if ids_to_add:
            try:
                self.yt.add_playlist_items(playlist_id, ids_to_add)
                self.stats.added_to_playlist += len(ids_to_add)
                logging.info(f"Successfully added final {len(ids_to_add)} songs to playlist")
            except Exception as e:
                logging.error(f"Failed to add final songs to playlist: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--delay', type=float, default=DEFAULT_REQUEST_DELAY, help="Delay in seconds between API requests (default: 0.5).")
    ...
    migrator = MusicMigrator(args.json, args.playlist, args.music_dir, args.delay)
    migrator.process()